...
# On constrained VMs, -NoUBA -MaxParallelActions 2 bounds compilation and disables detouring.
# UE 5.8 still uses the UBA scheduler; individual actions then report [NoUba].
param([string]$Config = "Development", [switch]$ProjectFiles,
    [ValidateRange(0,64)][int]$MaxParallelActions = 0, [switch]$NoUBA)
...
$buildArguments = @("ThanetEditor", "Win64", $Config, "-Project=$Proj", "-WaitMutex", "-FromMsBuild")
if ($MaxParallelActions -gt 0) { $buildArguments += "-MaxParallelActions=$MaxParallelActions" }
if ($NoUBA) { $buildArguments += "-NoUBA" }
& $dotnet $ubt @buildArguments 2>&1 |
