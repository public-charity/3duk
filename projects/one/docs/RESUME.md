...
Current retained candidate **`Saved/Phase1/arm_network_candidates/8abe75ce4814c0cf44ab`**:
all 246 documents; 168 width groups /420 common trims /15 end trims at five junctions.
**1,116 pass /359 fold /164 overlap /3 needs_geometry**: 471 new junction passes
versus original, **ZERO junction/body regressions**, all 1,411 changed bodies checked.
Fresh verification of three changed documents plus hash-proved reuse of 243 takes
10.054 s. `geometry_verification.json`, `verification/{corners,bodies}`.
Independent actual meshes: all 62 junction patches in the three pilot documents,
zero patch gaps, max corner seam gap **5.53224e-12 m**; all changed-end length
bounds hold. `mesh_seam_verification.json`. Actual mesh diagram inspected:
`actual_mesh_diagram.png` (three paired before/after plan views).

New tool
`Tools/diag/junction_arm_trim_candidates.py` uses actual end fields, observed
pre-reconciliation requests, exact baseline reproduction, all changed-end length
bounds, neighbour/body gates, full-document verification, immutable step files
and atomic state. 20 s search budget, at most two junctions per invocation.
Checks compare accumulated changes to the input as well as the current step.
Only full junction passes survive. **90 tool tests PASS** (8.113 s), including a
planted body regression that blocks retention. Real J14_12:5 independently
reproduced in 4.972 s /96 evaluations at
`junction_arm_trim_candidates/site_x14_y12/26f5d8cf589680052312`.

Eight-case pilot completed: **3/8 retained**, all from overlap cases (3/4);
four fold-status cases and J16_6:14 remain rejected. New passes J16_3:6,
J17_8:40 and J20_15:9 take 6.475–11.893 s each including process setup.
Logs `phase1_arm_search_pilot_batch{1,2}.log`. Selection/state:
`Saved/Phase1/arm_search_pilot_{selection,run}.json`; driver
`Saved/Phase1/run_arm_search_pilot.py` runs at most four jobs, resumes each result,
and each underlying tool checkpoints every junction. Repeat completed J14_12:5
invocation verifies checkpoint reuse (`phase1_arm_search_resume.log`).
Full tool suite completed: `Saved/phase1_arm_search_all_tool_tests.log`.
Documentation reads/writes must explicitly use UTF-8 on this Windows Python.

**Recovery:** committed `docs/checkpoints/phase1_25_geometry_selection.json` plus
`restore_geometry_selection.py --manifest projects/one/docs/checkpoints/phase1_25_geometry_selection.json`
recreates all 246 documents byte for byte in eight bounded batches. Verified at
`restored_geometry_selection/6e94d9f3dc433097cef9`; logs
`phase1_arm_pilot_restore_batchN.log`. No native/core change since checkpoint24.
526 junctions, continuation seams, terrain, structures and world acceptance remain
open. Production untouched; this is a recoverable geometry candidate.
...
Compare the signed triangle area in sweep (station, offset) coordinates with its
signed world XY area to find this reversal, regardless of emitted winding.
On four documents this exposes 42 junctions with mapping folds, including prior
passes. The private pavement-aware trim prototype reduces that to 23, but is not
ready for the shared renderer. Evidence `corner_topology/pavement_mapping_pilot.json`
and `pavement_mapping_trim_pilot.json`; helpers in `Saved/Phase1/probe_pavement_*.py`.
**Next: prove the stronger gate with a folded sweep fixture, apply it to both
geometry and terrain-contact QC, then regenerate baseline/candidate censuses.**
Do not promote a candidate based on the older normal-only pavement check.

Newest retained width candidate: **`connected_width_candidates/abc30fa75f91e84168c3`**.
210 groups / 261 splines, 63 need context, 16 held. All 246 documents complete;
independent preservation proof passes (15,422 splines, 253 source inputs).
`phase1_retained_width_independent_verification.log`. Its old-style geometry census
`corner_quality/40951a497d2c41c37763` stopped at 16/246 because the stronger gate
above is needed; no need to finish that obsolete census.

The first filtered candidate `cf50bdd76697def0e9e9` completed all 1,642 junctions:
1,284 pass / 106 fold / 249 overlap / 3 curve failures, 69 new passes but one
indirectly exposed regression at junction:11_13:3. Its cumulative held_groups.json
adds roads:30722752:0 to the original 15 held groups and produces `abc30...` above.
`phase1_filtered_width_comparison.log`, `phase1_width_holds_second.log`.

The generator's hashed `--hold-groups` option keeps complete continuation
components at original widths and rejects unknown/ineligible group IDs. 73 tool
tests pass. No renderer or production changes in this checkpoint.

The sections below are historical checkpoint notes; use the current state above.

#### Earlier twentieth-checkpoint progress (superseded)

Latest verified commit **`97f3f71`**, full initial width candidate. Generator now
accepts a hashed `--hold-groups` decision file and leaves each held continuation
component at its original width. The decision holds **15 groups / 16 splines**,
including two neighbouring groups that indirectly changed junction:17_10:0.
`connected_width_candidates/144f23ef8518eb1e0350/held_groups.json`, generated by
`Saved/Phase1/hold_width_regressions.py`; attribution is recorded per regression.
73 tool tests still pass (`phase1_filtered_width_tool_tests.log`). No native edits.

Filtered full candidate **`connected_width_candidates/cf50bdd76697def0e9e9`**:
211 candidate groups / 262 selected splines, 63 need context, 15 held for geometry.
All 246 documents materialized; independent complete-document verification and
new geometry census now running. Logs `phase1_filtered_width_candidate_batchNN.log`,
`phase1_filtered_width_independent_verification.log`,
`phase1_filtered_width_census_batchNN.log`. Census batch 01..04 running now.
Continue in <=16-document batches until all 246 are complete. Do not edit audit
dependencies while it runs. Read first census log for its new state fingerprint.

Reusable comparison helper now accepts census/candidate directory arguments:
`Saved/Phase1/compare_width_census.py <new-corner-quality-dir> <filtered-candidate-dir>`.
It checks all 1,642 junctions against unchanged original baseline; any remaining
