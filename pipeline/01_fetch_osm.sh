...
PROV="data/provenance/osm.json"
PY="${PY:-python3.14}"

mkdir -p data/raw data/derived data/provenance

BBOX=$("$PY" -c 'import json;print(",".join(str(v) for v in json.load(open("pipeline/config/margate.json"))["bbox_wgs84"]))')

if [ -s "$OSM" ]; then
  echo "01: $OSM present ($(du -h "$OSM" | cut -f1)) -- skipping fetch (rm it to force)"
else
  echo "01: fetching OSM for bbox $BBOX ..."
  BBOX="$BBOX" OUT="$OSM" ./tools/fetch_osm.sh
fi
