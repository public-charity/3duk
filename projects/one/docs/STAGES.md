...
sed -E 's/^([0-9a-f]{64}) \*?/\1  /' data/margate/regress/baseline_2026-09-08.sha256 | LC_ALL=C sort -k2 > data/margate/regress/baseline.norm; LC_ALL=C sort -k2 data/margate/regress/before.sha256 | diff - data/margate/regress/baseline.norm && echo IDENTICAL_TO_BASELINE
```
Expected: `N files hashed -> .../data/margate/regress/before.sha256` with N = the number of files under `data/margate/out` today (terrain 92 + networks 83 + massing 71 + coast 92 + furniture 14 + 2 `qa_*.json` + `unity/**`), then `NORMALISED`, then `IDENTICAL_TO_BASELINE` (same file set, same hashes as BRIEF §8's baseline — no product changed between the baseline and the first `sources/` edit).
...
C:/Users/Shadow/code/3duk-env/env/python.exe -c "import json; p=json.load(open('sources/provenance/thanet.osm.json')); print(p['query_sha256'] is not None, [s for s in p['query_selectors'] if 'railway' in s])"
...
C:/Users/Shadow/code/3duk-env/env/python.exe -c "import json; n=json.load(open('data/thanet/out/networks/networks_manifest.json')); l=json.load(open('data/thanet/out/networks/linear_manifest.json')); m=json.load(open('data/thanet/out/massing/massing_manifest.json')); c=json.load(open('data/thanet/out/coast/coast_manifest.json')); f=json.load(open('data/thanet/out/furniture/qa_furniture.json')); print('06', n['clip']['keep'], n['vertices_outside_clip'], n['junctions_outside_clip'], n.get('segments'), n.get('junctions')); print('11 rail', l['layers']['rail']['segments'], l['layers']['rail']['tiles'], l['layers']['rail']['length_km'], l['layers']['rail']['by_class'], 'gauge_defaulted', l['layers']['rail']['gauge_defaulted'], 'outside_clip', l['layers']['rail']['vertices_outside_clip']); print('11 barriers', l['layers']['barriers']['segments'], l['layers']['barriers']['by_class'], l['ways_skipped_by_class']); print('07', m['outside_clip'], m.get('height_calib',{}).get('source')); print('09', len(c['tiles_clipped']), c['clipped_cells'], c['bands_note'][:40]); print('10', f['outside_clip'])"
