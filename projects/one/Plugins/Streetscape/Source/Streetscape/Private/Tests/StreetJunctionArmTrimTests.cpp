...
bool FStreetArmTrimParityTest::RunTest(const FString&)
{
	for (const FString Fixture : { FString(TEXT("junction_arm_trims")), FString(TEXT("junction_collapsed_pavement")), FString(TEXT("junction_connector_bend")), FString(TEXT("junction_connector_reverse")) })
	{
		...
		if (!BuildJunctionSite(*this,*Fixture,Doc,Built)) return false;
		const auto Original = JunctionFixture(*this,*Fixture);
		...
		if (!Reference->TryGetObjectField(Fixture,Cases)) { AddError(TEXT("arm trim reference case missing")); return false; }
		for (const auto& Def : Doc.Splines)
		{
			...
			Compare(*Row,Fixture+TEXT(".")+Def.Id+TEXT(".s"),TEXT("s"),Samples.S,1e-9);
			Compare(*Row,Fixture+TEXT(".")+Def.Id+TEXT(".trim"),TEXT("trim"),{Samples.STrim[0],Samples.STrim[1]},1e-9);
			...
			for (const auto& Part : Parts)
			{
				...
				if (Fixture==TEXT("junction_collapsed_pavement"))
				{
					for (int32 I=0; I<Mesh.F.Num(); ++I)
					{
						const auto& F=Mesh.F[I];
						if (!Mesh.GroupNames[Mesh.Grp[I]].ToString().StartsWith(TEXT("corner_pavement:"))) continue;
						if (FMath::Min3(Mesh.VH[F[0]],Mesh.VH[F[1]],Mesh.VH[F[2]]) < -1e-8) continue;
						TestTrue(TEXT("opening corner pavement faces up"),Mesh.FaceNormal(I).Z>=-1e-8);
					}
				}
				...
				const FString Label=Fixture+TEXT(".")+Def.Id+TEXT(".")+Part.Key;
