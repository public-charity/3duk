...
for p in sorted(by_doc):
    ...
    for (k, sid, layer, cls) in by_doc[p]:
        ...
        for rule in ("bi", "tri"):
            ...
            per_station_min = np.where(good, cl, np.inf).min(axis=1)
            valid = np.isfinite(per_station_max) & np.isfinite(per_station_min)
            if not valid.any():
                continue
            rows.append(dict(id=sid, layer=layer, cls=cls, rule=rule, L=sp.length,
                             n_station=int(valid.sum()), pen=per_station_max[valid],
                             drop=per_station_min[valid], s=sp.s[valid]))
        built += 1
...
def summarise(rule):
    ...
    fl = -np.concatenate([r["drop"] for r in R])
    out["float_gt_0125_fraction"] = float((fl > 0.125).mean())
    out["float_gt_0125_stations"] = int((fl > 0.125).sum())
    out["float_max_m"] = float(fl.max())
    out["float_p95_m"] = float(np.percentile(fl, 95))
    out["float_p99_m"] = float(np.percentile(fl, 99))
    if hit.any():
        pp = pen[hit]
