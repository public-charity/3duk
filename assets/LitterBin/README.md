...
writes an FBX + albedo pair into `assets/<Name>/`. The Unity-side prefab builder that
turned the pair into URP materials, an LODGroup and a collider lived in the game project and
is no longer here -- these files are the shipped asset and the recipe that produces it.

```bash
blender --background --factory-startup --python assets/LitterBin/build_litterbin.py
# -> SM_LitterBin_Jubilee110.fbx + T_LitterBin_D.png, overwriting the pair in this folder
```
...
| Provenance | `assets/LitterBin/provenance.json` |
...
- **The asset is FBX, not glTF**, because the consuming Unity project had no glTF importer.
  Export from Blender with `apply_scale_options='FBX_SCALE_ALL'` and
