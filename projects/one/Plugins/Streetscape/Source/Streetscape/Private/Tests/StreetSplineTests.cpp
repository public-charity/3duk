...
// ---------------------------------------------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FStreetSplineElevationProfileTest, "Streetscape.Spline.ElevationProfile", kFlags)
bool FStreetSplineElevationProfileTest::RunTest(const FString& Parameters)
{
	FStreetHeightfield Terrain = FStreetHeightfield::FromFunction([](double X, double Y)
		{ return 20.0 - 10.0 * std::exp(-FMath::Square((X - 50.0) / 10.0)); },
		FVector2d(128.0, 128.0), 1.0, 128.0, FVector2d::ZeroVector, FVector2d(0.0, -64.0));
	FStreetSiteDoc Doc; FStreetSamples Sp;
	if (!BuildFixture(*this, TEXT("elevation_profile_100"), &Terrain, Doc, Sp)) return false;
	TestTrue(TEXT("profile knot is a shared mandatory station"), Sp.S.Contains(12.3) && Sp.S.Contains(67.8));
	double LowestRaw = 1000, MaxDeckError = 0;
	for (int32 I = 0; I < Sp.Num(); ++I)
	{
		LowestRaw = FMath::Min(LowestRaw, Sp.ZRaw[I]);
		MaxDeckError = FMath::Max(MaxDeckError, FMath::Abs(Sp.ZRef[I] - (20.0 + 0.01 * Sp.S[I])));
	}
	TestTrue(TEXT("raw survey still contains the valley"), LowestRaw < 11.0);
	TestTrue(TEXT("modelled deck is straight through the valley"), MaxDeckError < 1e-12);
	TestEqual(TEXT("bank knot survives unchanged"), InterpS(Sp, Sp.BankDeg, 12.3), 1.0, 1e-12);
	FStreetSiteDoc Round;
	TArray<FString> Problems;
	TestTrue(TEXT("profile JSON roundtrip reads"), FStreetscapeJson::ReadSite(FStreetscapeJson::WriteSite(Doc), Round, Problems));
	TestEqual(TEXT("profile JSON roundtrip preserves knots"), Round.Splines[0].ElevationProfile.Num(), 4);
	TestEqual(TEXT("profile JSON roundtrip preserves bank"), Round.Splines[0].ElevationProfile[2].BankDeg, -1.0);
	FString Error;
	Doc.Splines[0].ElevationProfile.Last().SM = 99;
	TestFalse(TEXT("changed horizontal length invalidates model"), FStreetSplineMath::Build(Doc.Splines[0], Doc.Profiles, nullptr, Sp, &Error));
	Doc.Splines[0].ElevationProfile.Last().SM = 100;
	Doc.Splines[0].ElevationProfile[1].SM = 0;
	TestFalse(TEXT("duplicate profile stations fail"), FStreetSplineMath::Build(Doc.Splines[0], Doc.Profiles, nullptr, Sp, &Error));
	return true;
}
...
bool FStreetSplineNumpyParityTest::RunTest(const FString& Parameters)
{
	...
	Cases.Add({ TEXT("elevation_profile_100"), TEXT("elevation_profile_100"), TerrainFor(TEXT("straight_100")) });
