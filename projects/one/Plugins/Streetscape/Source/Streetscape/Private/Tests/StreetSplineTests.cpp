...
// ---------------------------------------------------------------------------------------------------------------
// Optional: bit-level parity against a numpy dump (scratch file written by the numpy prototype; path in the
...
bool FStreetSplineNumpyParityTest::RunTest(const FString& Parameters)
{
	...
	{
		AddInfo(TEXT("STREETSCAPE_PARITY_JSON not set - parity check skipped"));
		return true;
	}
